...
function lengthOfRailway(sounds) {
  ...
  let isHigh = false;
  let count = 0;
  while (sounds.indexOf(soundHit) !== -1) {
    //     console.log(sounds)
    if (sounds.indexOf(soundOutIn) >= 0 && sounds.indexOf(soundOutIn) < sounds.indexOf(soundHit)) {
      isHigh = isHigh ? false : true;
      sounds = sounds.slice(0, sounds.indexOf(soundOutIn)) + sounds.slice(soundOutIn.length + sounds.indexOf(soundOutIn))
    } else {
      isHigh ? count += 20 : count += 10;
      sounds = sounds.slice(0, sounds.indexOf(soundHit)) + sounds.slice(soundHit.length + sounds.indexOf(soundHit))
    }
  }
  return (count);
}
