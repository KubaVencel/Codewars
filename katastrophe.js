...
function strongEnough(earthquake, age) {
  earthquake = earthquake.map(wave => wave.reduce((acc, curr) => acc + curr,
    0));
  const mag = earthquake.reduce((acc, curr) => acc * curr,
    1);
  let strength = 1000;
  for (let decay = 1; decay <= age; decay++) {
    strength = strength - (strength / 100);
  };
  if (strength > mag) return "Safe!"
  else return "Needs Reinforcement!"
}
