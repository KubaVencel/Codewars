...
The clock page replacement algorithm keeps a circular list of slots in memory, with the iterator pointing at the last examined slot in the list. When a page fault occurs and no empty slots exist, the algorithm checks the R-bit (referenced) of the page pointed by the iterator: if the R-bit is 0 the page pointed is removed from the memory, the new page is inserted and the iterator is moved forward, else if the R-bit is 1 it is set to 0 and the iterator is moved forward (this process is repeated until a page with an R-bit equal to 0 is found). Note that the R-bit is set to 1 when a page that is already in memory is referenced again.
...
So, at the end we have the list [1, 6, 5], which is what you have to return. If not all the slots in the memory get occupied after applying the algorithm, fill the remaining slots (at the end of the list) with -1 to represent emptiness (note that the IDs will always be >= 1).

*/

function clock(n, referenceList) {
  let bits = Array(n);
  let pages = Array(n).fill(-1);
  let pageIndexes = {};
  let bitIndex = 0;
  for (const page of referenceList) {
    if (pageIndexes[page] !== undefined) {
      bits[pageIndexes[page]] = 1;
      continue;
    }
    for (let i = bitIndex; i < Infinity; i++) {
      i %= n;
      if (!bits[i]) {
        bits[i] = 0;
        delete pageIndexes[pages[i]];
        pages[i] = page;
        pageIndexes[page] = i;
        bitIndex = i + 1;
        break;
