...
function drawSpider(legSize, bodySize, mouth, eye) {
  console.log(legSize, bodySize, mouth, eye);
  let legs = legSize === 1 ? "^" :
    legSize === 2 ? "/\\" :
      legSize === 4 ? "╱╲" : "";
  let bodyLeft = Array(bodySize).fill("(").join("");
  let bodyRight = Array(bodySize).fill(")").join("");
  let eyes = Array(Math.pow(2, bodySize - 1)).fill(eye).join("");
  return (legSize == 3 ? "/╲" : legs) +
    bodyLeft + eyes + mouth + eyes + bodyRight +
    (legSize == 3 ? "╱\\" : legs);
}
