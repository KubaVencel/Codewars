...
function fruitPack(orders) {
  return orders.map(x => {
    let pallet = "", box = "", bag = ""
    for (let arr of x.match(/(\d+)(.)/g)) {
      let d = +arr.slice(0, -1), c = arr.slice(-1)
      pallet += `[${c}]`.repeat(Math.floor(d / 50))
      box += `{${c}}`.repeat(Math.floor(d % 50 / 10))
      bag += d % 10 ? `(${c.repeat(d % 10)})` : ""
    }
    let maxi = Math.max(...[bag, box, pallet].map(s => s.length))
    return [bag, box, pallet].map(x => x.padStart(maxi, '-'))
  })
}
