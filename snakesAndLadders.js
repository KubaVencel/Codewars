...
function snakesAndLadders(board, dice) {
  let square = 0; // Initialize the starting square to 0
  for (let die of dice) { // Iterate over each dice throw
    if (square + die >= board.length) { // If moving forward exceeds the board length, continue to the next iteration
      continue;
    }
    square += die; // Move forward according to the dice value
    square += board[square]; // Check the value at the new square and adjust the position accordingly
  }
  return square; // Return the final square position
}
