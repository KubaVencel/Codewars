...
*/
function rotate(str) {
  let res = [];   // Create an empty array to store the rotated strings
  let s = str;    // Assign the input string to a temporary variable s

  for (let i = 0; i < str.length; i++) {
    s = s.slice(1) + s[0];  // Rotate the string by moving the first character to the end
    res.push(s);            // Add the rotated string to the result array
  }

  return res;  // Return the array containing all the rotated strings
}
