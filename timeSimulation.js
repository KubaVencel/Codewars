...
class SimTime {
  ...
  update(current_realtime) {
    if (current_realtime < this._realtime) throw Error()

    const delta_time = current_realtime - this._realtime

    this._time += this._speed * delta_time
    this._realtime = current_realtime
  }
}
